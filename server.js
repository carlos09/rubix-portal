...
app.get('*', function(req, res, next) {
  ...
  try {
    Router.run(routes(), location, function(e, i, t) {
      i.params.server_data = "Server Data!";

      var str = React.renderToString(
                  React.createElement(Router, i));

      if(isRTL) {
        rtl = rtl.replace(new RegExp('{server_data}', 'g'), i.params.server_data);
        res.send(rtl.replace(new RegExp('{container}', 'g'), str));
      } else {
        ltr = ltr.replace(new RegExp('{server_data}', 'g'), i.params.server_data);
